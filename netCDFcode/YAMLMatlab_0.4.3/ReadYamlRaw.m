...
function result = load_yaml(inputfilename, nosuchfileaction, treatasdata)
    ...
    if ~tadf
        ...
        if isempty(char(filepath))  % sb added char array here, because an empty string is read as not empty 
            pathstore = cd();
        else
            pathstore = cd(char(filepath));
        end;
    end;
    try
        if ~tadf
            result = scan(yaml.load(fileread([char(filename), char(fileext)])));
        else
